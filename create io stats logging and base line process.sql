...
--	Date:					11/09/2018
--	Description:			The capture and comparison of baseline IO activity in a given SQL instance for
--							base lining.
--
--	Credits to:				https://www.mssqltips.com/sqlservertip/2350/creating-io-performance-snapshots-to-find-sql-server-performance-problems/
--
-- Useage:
--
-- First we must create tables to store the I/O statistics. The following code will create two tables. 
-- You can put this in the master database or whatever database you prefer.
--
-- #############################################################################################################
--
-- create the table for snapshot sequence generation
--
-- ###################################################

use dbadmin;

CREATE TABLE dba.io_snapshots
(snap_id INT IDENTITY NOT NULL,
snapshot_creation_date DATETIME NOT NULL )
GO
ALTER TABLE dba.io_snapshots ADD CONSTRAINT PK_io_snapshots PRIMARY KEY (snap_id)
GO

-- ###################################################
--
-- create the table for hold the io statistics
--
-- ###################################################

CREATE TABLE dba.io_snapshots_statistics
...
GO
ALTER TABLE dba.io_snapshots_statistics ADD CONSTRAINT PK_io_snapshots_statistics
PRIMARY KEY (snap_id,[db_id], [file_id])
GO
ALTER TABLE dba.io_snapshots_statistics ADD CONSTRAINT FK_io_snapshots_statistics_io_snapshots
FOREIGN KEY (snap_id) REFERENCES dba.io_snapshots (snap_id)
GO

alter table dba.io_snapshots_statistics add snap_date datetime;
ALTER TABLE dba.io_snapshots_statistics ADD CONSTRAINT DFC_io_snapshots_statistics_date DEFAULT GETDATE() FOR snap_date;

-- ########################################################################################################################
--
-- The first SP (usp_io_vf_stats_snap) inserts a new I/O snapshot of statistics each time it is run.
-- create the sp to then populate the table...
--
-- Useage:	Run the collection sproc via a scheduled SQL Agent job on your required schedule
--
--	exec [dba].[usp_dba_io_vf_stats_snap]
--
-- ########################################################################################################################

CREATE PROC [dba].[usp_dba_io_vf_stats_snap]
AS
BEGIN
	...
	INSERT INTO dba.io_snapshots_statistics 
	(snap_id,
	[db_id],
	[file_id],
	database_name ,
...
-- ########################################################################################################################
-- 
--  create the sproc to compare the snapshots.
--
...
--
-- Useage:	exec [dba].[usp_dba_compare_io_stats_snaps] 1,2
--
-- check for latest id to compare against
--
--	select max(snap_id),snap_date from dba.io_snapshots_statistics 
--	group by snap_Date
--
-- ########################################################################################################################
--
CREATE PROC [dba].[usp_dba_compare_io_stats_snaps] 
